# Databricks notebook source exported at Sun, 11 Sep 2016 12:27:35 UTC
# MAGIC %md
...
# MAGIC %sql
# MAGIC -- TO DO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC SELECT AP AS PRESURE, PE AS POWER FROM power_plant
...
vectorizer.setInputCols(inputCols)
vectorizer.setOutputCol(outputCol)

output = vectorizer.transform(datasetDF)
print(output.select("features", "PE").first())
...
# TEST
Test.assertEquals(round(rmseRF, 2), 3.66, "Incorrect value for rmseRF")
Test.assertEquals(round(r2RF, 2), 0.95, "Incorrect value for r2RF")
