# Databricks notebook source exported at Sun, 11 Sep 2016 12:13:00 UTC
# MAGIC %md
...
def simpleTokenize(string):
    ...
    """
    return filter(lambda string: string != '', re.split(split_regex, string.strip().lower()))
...
def tokenize(string):
    ...
    """
    split_string = filter(lambda x: x!='', re.split(split_regex, string.lower()))
    final_string = [item for item in split_string if item not in stopwords]
    return final_string
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #return vendorRDD.takeOrdered(1, key = lambda (id, tokens): -len(tokens)) 
...
def tf(tokens):
    ...
    for token in tokens:
      length = length+1
      if token in tf_dict:
        tf_dict[token] = tf_dict[token] +1
      else:
        tf_dict[token] = 1
          
    for item in tf_dict:
      tf_dict[item] = tf_dict[item]/float(length)
...
# TODO
#smallIDFTokens = idfsSmall.sortBy(lambda y: y[1]).take(11)
smallIDFTokens = idfsSmall.takeOrdered(11, lambda y: y[1])
...
idfsToCountRow = idfsSmall.map(lambda x: (x[0],x[1]))
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda x:(" ".join([x[1],x[0]]),x[2]))

trueDupsRDD = (sims
               .join(goldStandard).map(lambda x:(x[0],x[1][0])))
trueDupsCount = trueDupsRDD.count()
avgSimDups = trueDupsRDD.map(lambda x:float(x[1])).mean()


nonDupsRDD = (sims
              .subtractByKey(trueDupsRDD))
avgSimNon = nonDupsRDD.map(lambda x:float(x[1])).mean()
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = amazonFullRecToToken.union(googleFullRecToToken).map(lambda x:(x[0],tf(x[1])))
...
amazonWeightsRDD = amazon.map(lambda x:(x[0],tfidf(tokenize(x[1]),idfsFullBroadcast.value)))
googleWeightsRDD = google.map(lambda x:(x[0],tfidf(tokenize(x[1]),idfsFullBroadcast.value)))
...
print 'There are %s Amazon norms and %s Google norms.' % ((amazonNorms.count()), (googleNorms.count()))
...
def invert(record):
    ...
    """
    pairs=[(token, record[0]) for token in record[1].keys()]
    return (pairs)
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(lambda x:swap(x))
                .reduceByKey(lambda x,y:(x+","+y)).map(lambda x:(x[0],x[1].split(",")))
                .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    tokenamazonweight = amazonWeightsBroadcast.value[amazonRec]
    tokengoogleweight = googleWeightsBroadcast.value[googleRec]
    amazonIDnorm = amazonNormsBroadcast.value[amazonRec]
    googleURLnorm = googleNormsBroadcast.value[googleRec]
    s = sum([tokenamazonweight[token]*tokengoogleweight[token] for token in tokens])
    value = s*((amazonIDnorm)**(-1))*((googleURLnorm)**(-1))
