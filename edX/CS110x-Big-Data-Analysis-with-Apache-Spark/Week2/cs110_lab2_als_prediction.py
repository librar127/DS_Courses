# Databricks notebook source exported at Sun, 11 Sep 2016 12:27:20 UTC
# MAGIC %md
...
# Note: movie_names_df is a temporary variable, used only to separate the steps necessary to create the movie_names_with_avg_ratings_df DataFrame.
...
# From ratingsDF, create a movie_ids_with_avg_ratings_df that combines the two DataFrames
avg_rating_df = training_df.groupBy('movieId').agg(F.avg(ratings_df.rating).alias("average"))
...
test_for_avg_df = test_df.withColumn('prediction', F.lit(","), avg_rating_df.average)
