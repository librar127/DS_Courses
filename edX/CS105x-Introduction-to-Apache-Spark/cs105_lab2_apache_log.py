# Databricks notebook source exported at Sun, 11 Sep 2016 12:33:13 UTC
# MAGIC %md
...
#
# This could be done in one line with a Python list comprehension, but we're keeping it simple for those who don't know Python very well.
'''
exprs = []
...
host_more_than_10_df = (host_sum_df
                        ...
                        .select(host_sum_df['host']))#, host_sum_df['count']))
...
daily_hosts_df = day_group_hosts_df.groupBy('day').count().sort('day')
...
daily_hosts_df_counts = (daily_hosts_df
                .select('day', 'count')
                .map(lambda r: (r[0], r[1]))
                .collect())

days_with_hosts, hosts = zip(*daily_hosts_df_counts)
'''
...
for <FILL IN>:
  <FILL IN>
'''
days_with_hosts = (list(days_with_hosts))
hosts = (list(hosts))
...
test_days.remove(2)
test_hosts = [2582, 3222, 4190, 2502, 2537, 4106, 4406, 4317, 4523, 4346, 2864, 2650, 4454, 4214, 4340, 4385, 4168, 2550, 2560, 4134, 4456]
Test.assertEquals(days_with_hosts, test_days, 'incorrect days')
Test.assertEquals(hosts, test_hosts, 'incorrect hosts')
...
total_req_per_day_df = logs_df.groupBy(dayofmonth(logs_df['time']).alias('day')).count().alias('count1')
daily_hosts_df1 = daily_hosts_df.select("day", daily_hosts_df["count"].alias("count2"))
joinedDF = total_req_per_day_df.join(daily_hosts_df1, "day")

avg_daily_req_per_host_df_ = joinedDF.select('day', joinedDF['count']/joinedDF['count2'])



avg_daily_req_per_host_df = (
  avg_daily_req_per_host_df_.select('day', avg_daily_req_per_host_df_['(count / count2)'].alias('avg_reqs_per_host_per_day'))
)
...
# TODO: Replace <FILL IN> with appropriate code
days_with_avg_counts = (avg_daily_req_per_host_df
                .select('day', 'avg_reqs_per_host_per_day')
                .map(lambda r: (r[0], r[1]))
                .collect())

days_with_avg, avgs = zip(*days_with_avg_counts)

days_with_avg = list(days_with_hosts)
avgs = list(avgs)
...
errors_by_date_sorted_df = not_found_df.groupBy(dayofmonth(not_found_df['time']).alias('day')).count().sort('day')
...
days_with_errors_404 = [d['day'] for d in errors_by_date_sorted_df.select('day').collect()]
errors_404_by_day = [d['count'] for d in errors_by_date_sorted_df.select('count').collect()]
...
top_err_date_df = errors_by_date_sorted_df.sort('count').sort('count', ascending=False)
...
hour_records_sorted_df = not_found_df.select(hour(not_found_df['time']).alias('hour')).groupby('hour').count().sort('hour')
...
hours_with_not_found = [h['hour'] for h in hour_records_sorted_df.select('hour').collect()]
not_found_counts_per_hour = [h['count'] for h in hour_records_sorted_df.select('count').collect()]
